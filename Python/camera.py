...
i_http_port = 80
i_rtsp_port = 554
i_rtsp_stream = 'stream2'
i_width = 1280
i_height = 720
i_display_width = 1920
i_display_height = 1280
i_scaling = 2 # 2x
...
cv2.moveWindow('Capturing',400,0)#Init window position
...
while True:
    ...
    if len(Json_data["AiEngine"]) != 0: #Find AiEngine token
        for j in range(len(Json_data["AiEngine"])): #Loop for display bouding box and plate
            ...
            i_x1 = int(int(Json_data["AiEngine"][j]["x"])*i_width/i_display_width)
            i_y1 = int(int(Json_data["AiEngine"][j]["y"])*i_height/i_display_height)
            i_x2 = i_x1 + int(int(Json_data["AiEngine"][j]["w"])*i_width/i_display_width)
            i_y2 = i_y1 + int(int(Json_data["AiEngine"][j]["h"])*i_width/i_display_height)
            ...
            if Json_data["AiEngine"][j]["label_name"] == 'person':
                ...
                i_confidence = str(Json_data["AiEngine"][j]["confidence"])
                cv2.putText( frame , i_label + ",confidence:" + i_confidence , ( i_x1 , i_y1 - 5 ) , cv2.FONT_HERSHEY_SIMPLEX , 1 , ( 0 , 0 , 255 ) , 1 , cv2.LINE_AA ) #test_label
                #cv2.putText( frame , i_confidence , ( i_x1 , i_y2 + 25 ) , cv2.FONT_HERSHEY_SIMPLEX , 1 , ( 0 , 0 , 255 ) , 1 , cv2.LINE_AA ) #test_label
            elif Json_data["AiEngine"][j]["label_name"] == 'L._Plate_USA':
                i_label = str(Json_data["AiEngine"][j]["properties"]["plate"])
                i_confidence = str(Json_data["AiEngine"][j]["confidence"])
                i_country = str(Json_data["AiEngine"][j]["properties"]["country"])
                i_area = str(Json_data["AiEngine"][j]["properties"]["area"])
                cv2.putText( frame , i_label + ",confidence:" + i_confidence, (i_x1,i_y1-5) , cv2.FONT_HERSHEY_SIMPLEX , 1 , (0, 0, 255) , 1 , cv2.LINE_AA ) #test_label
                cv2.putText( frame , i_area+ "," + i_country , (i_x1,i_y2+25) , cv2.FONT_HERSHEY_SIMPLEX , 1 , (0, 0, 255) , 1 , cv2.LINE_AA ) #test_label
